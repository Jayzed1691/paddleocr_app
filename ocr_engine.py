...
class OCREngine:
    ...
    def process_image(self, image: Union[str, Path, Image.Image, np.ndarray]) -> List[Any]:
        ...
        try:
            ...
            logger.info(
                f"OCR completed successfully, found {len(result[0]) if result[0] else 0} text blocks"
            )
            return result
    ...
    def process_images(
        self, images: List[Union[str, Path, Image.Image, np.ndarray]]
    ) -> List[List[Any]]:
        """
    ...
    def format_as_json(self, ocr_results: List[List[Any]]) -> str:
        ...
        Returns:
            JSON string
        """
        json_data = {"total_pages": len(ocr_results), "pages": []}
    ...
    def get_statistics(self, ocr_results: List[List[Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": (
                sum(avg_confidence) / len(avg_confidence) if avg_confidence else 0.0
            ),
